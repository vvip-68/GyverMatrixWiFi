...
#define CURRENT_LIMIT 6000    // лимит по току в миллиамперах, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
...
byte autoBrightnessMin = 0;        // Минимальный уровень яркости при автоматической регулировке
...
int8_t AM1_effect_id = -2;           // Режим 1 по времени - ID эффекта или -2 - выключено; 0 - выключить матрицу (черный экран); 1 - случайный
...
int8_t AM2_effect_id = -2;           // Режим 2 по времени - ID эффекта или -2 - выключено; 0 - выключить матрицу (черный экран); 1 - случайный
