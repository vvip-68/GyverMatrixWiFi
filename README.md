...
#### Изменения по справнению с исходным проектом:
 ...
 - Другие типы контроллеров (Arduino Mega + WiFi, Wemos D1) - не тестировались.
...
О: Нет, все правильно. IP матрицы - 192.168.4.1; Адрес 192.168.4.2 получает ваш смартфон при подключении к точке доступа. Когда в приложении для подключения вы указываете адрес 192.168.4.2 - вы подключаете ваш смартфон с самому себе, а не к матрицы. Естественно никакое управление матрицей работать не будет.   
