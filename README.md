...
## Важно
Если проект не собирается (ошибки компиляции) или собирается, но работает неправильно (например вся матрица светится белым и ничего не происходит) - проверьте версии библиотек. Данный проект изначально разрабатывался и собирался для версии библиотек поддержки плат ESP версии 2.5.0 и библиотеки FastLED версии 3.2.0;

* Библиотеки ESP весии 2.5.2 и FastLED 3.2.8 - проект компилируется, но после загрузки вся матрица светится белым.
* Библиотеки ESP весии 2.5.2 и FastLED 3.2.2, 3.2.7 - проект не компилируется, очень много ошибок в библиотеках   
* Библиотеки ESP весии 2.5.2 и FastLED 3.2.0, 3.2.1, 3.2.3, 3.2.4, 3.2.5, 3.2.6 - проект не компилируется, ошибка переопределения типа  

typedef bool boolean;   

Для устранения ошибки компилляции c версиями FastLED 3.2.0, 3.2.1, 3.2.3, 3.2.4, 3.2.5, 3.2.6 - в файле библиотеки в папке "FastLED/platforms/esp/8266/led_sysdefs_esp8266.h" в строке 15 закомментируйте переопределение типа   

// typedef uint8_t boolean;    

После этого компилляция выпролняется успешно, загруженная прошивка работает.
